...
import type { LoginBody, RegisterBody } from '../validations/auth.validator.ts';
...
export const authService = {
  register: async (body: RegisterBody) => {
    const existingUser = await authRepository.findUserByUsernameOrEmail(body.username, body.email);
  ...
  refresh: async (refreshToken: string) => {
    ...
    return await createAuthTokens(userId);
  },
